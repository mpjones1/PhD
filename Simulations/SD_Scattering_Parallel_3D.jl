...
function run(phi0,N,chi_c,chi_s,chi,ICn,l,v0,lambda,dt,dts,dx,nsteps,nsnaps,rep,nthreads1D,nblocks1D)
    ...
    for t = 1:1:nsteps # loop over time steps
        # NOTE: if wanting to simulate dissolution, one option is to include an if-else statement here to change the value of chi from chi_i to chi_f after a certain number of time steps
    ...
    writedlm("../Scattering_Data/ScD_Phi0=$phi0 N=$N chi=$chi ICn=$ICn dx=$dx dt=$dt nsteps=$nsteps dts=$dts L=$l v0=$v0 lam=$lambda 3D SD 2DFT rep=$rep.txt", SD) 
    writedlm("../Scattering_Data/PCF_Phi0=$phi0 N=$N chi=$chi ICn=$ICn dx=$dx dt=$dt nsteps=$nsteps dts=$dts L=$l v0=$v0 lam=$lambda 3D SD 2DFT rep=$rep.txt", PC) 
end
